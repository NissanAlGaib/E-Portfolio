...
function renderSkills(skills) {
  ...
  if (technicalSkills.length > 0) {
    technicalContainer.innerHTML = technicalSkills.map(skill => renderSkillCard(skill)).join('');
  } else {
  ...
  if (softSkills.length > 0) {
    softContainer.innerHTML = softSkills.map(skill => renderSkillCard(skill)).join('');
  } else {
...
function renderSkillCard(skill) {
  const iconHtml = skill.icon 
    ...
    : `<div class="w-12 h-12 rounded-lg bg-gradient-to-r from-blue-start to-purple-end mb-3 flex items-center justify-center text-2xl font-bold">${skill.skill_name.charAt(0)}</div>`;
  ...
  return `
    <div class="bg-glass border border-gray-700 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-shadow duration-300">
      ...
      ${skill.category ? `<p class="text-sm text-gray-400 mb-3">${skill.category}</p>` : ''}
      
      <div class="mb-4">
        <div class="flex justify-between items-center mb-1">
          <span class="text-sm text-gray-300">Proficiency</span>
          <span class="text-sm text-white font-bold">${skill.proficiency}%</span>
        </div>
        <div class="w-full bg-gray-700 h-2 rounded-full overflow-hidden">
          <div class="bg-gradient-to-r from-blue-start to-purple-end h-2 rounded-full transition-all duration-500" style="width: ${skill.proficiency}%"></div>
        </div>
      </div>
      ...
      ${skill.description ? `<p class="text-gray-300 text-sm mt-3">${skill.description}</p>` : ''}
    </div>
  `;
}
