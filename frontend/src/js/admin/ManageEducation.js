function loadEducationAdmin() {
  fetch("../../backend/api/education_api.php?user_id=1")
    .then((res) => res.json())
    .then((data) => {
    ...
    return `
      <div class="bg-glass border border-gray-700 rounded-lg p-4 flex items-start justify-between">
        <div class="flex items-start space-x-4 flex-grow">
          ${logoUrl ? `<img src="${logoUrl}" alt="${edu.institution_name}" class="w-16 h-16 object-contain rounded">` : ''}
          <div class="flex-grow">
            <h3 class="text-white font-bold text-lg">${edu.institution_name}</h3>
            <p class="text-gray-300">${edu.degree}${edu.field_of_study ? ' in ' + edu.field_of_study : ''}</p>
            <div class="flex items-center space-x-2 mt-1">
              ${edu.location ? `<span class="text-xs text-gray-500">${edu.location}</span>` : ''}
              ${startDate ? `<span class="text-xs text-gray-500">${startDate} - ${endDate}</span>` : ''}
              ${edu.gpa ? `<span class="text-xs text-gray-500">GPA: ${edu.gpa}</span>` : ''}
            </div>
            ${edu.description ? `<p class="text-gray-400 text-sm mt-2">${edu.description}</p>` : ''}
          </div>
        </div>
        <button onclick="deleteEducation(${edu.id})" class="ml-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">Delete</button>
      </div>
    `;
  }).join('');
}
...
function handleAddEducation(e) {
  ...
  fetch("../../backend/api/education_api.php", {
    method: "POST",
    body: formData,
  })
    .then((res) => res.json())
